...
describe('resource following', () => {
  ...
  test.skip('list', async () => {
    const responsePromise = client.users.following.list('182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e');
  ...
  test.skip('listPosts', async () => {
    const responsePromise = client.users.following.listPosts('182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e');
    const rawResponse = await responsePromise.asResponse();
    expect(rawResponse).toBeInstanceOf(Response);
    const response = await responsePromise;
    expect(response).not.toBeInstanceOf(Response);
  ...
  test.skip('listPosts: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(
      client.users.following.listPosts(
        '182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e',
        { after: 'after', before: 'before', limit: 0, offset: 0 },
        { path: '/_stainless_unknown_path' },
      ),
