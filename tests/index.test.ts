...
import { APIPromise } from 'bandlab-sdk/core/api-promise';

import util from 'node:util';
import BandlabSDK from 'bandlab-sdk';
import { APIUserAbortError } from 'bandlab-sdk';
...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    beforeEach(() => {
      process.env = { ...env };
      process.env['BANDLAB_SDK_LOG'] = undefined;
    });
    ...
    const forceAPIResponseForClient = async (client: BandlabSDK) => {
    ...
    test('debug logs when log level is debug', async () => {
      ...
      const client = new BandlabSDK({ logger: logger, logLevel: 'debug', bearerToken: 'My Bearer Token' });
    ...
    test('default logLevel is warn', async () => {
      const client = new BandlabSDK({ bearerToken: 'My Bearer Token' });
    ...
    test('debug logs are skipped when log level is info', async () => {
      ...
      const client = new BandlabSDK({ logger: logger, logLevel: 'info', bearerToken: 'My Bearer Token' });
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      process.env['BANDLAB_SDK_LOG'] = 'debug';
      const client = new BandlabSDK({ logger: logger, bearerToken: 'My Bearer Token' });
    ...
    test('warn when env var level is invalid', async () => {
      ...
      process.env['BANDLAB_SDK_LOG'] = 'not a log level';
      const client = new BandlabSDK({ logger: logger, bearerToken: 'My Bearer Token' });
      expect(client.logLevel).toBe('warn');
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'BANDLAB_SDK_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
    ...
    test('client log level overrides env var', async () => {
      ...
      process.env['BANDLAB_SDK_LOG'] = 'debug';
      const client = new BandlabSDK({ logger: logger, logLevel: 'off', bearerToken: 'My Bearer Token' });
  ...
  describe('baseUrl', () => {
    ...
    afterEach(() => {
      process.env['BANDLAB_SDK_BASE_URL'] = undefined;
    });

    test('explicit option', () => {
      const client = new BandlabSDK({ baseURL: 'https://example.com', bearerToken: 'My Bearer Token' });
      expect(client.baseURL).toEqual('https://example.com');
    });

    test('env variable', () => {
      process.env['BANDLAB_SDK_BASE_URL'] = 'https://example.com/from_env';
      const client = new BandlabSDK({ bearerToken: 'My Bearer Token' });
      expect(client.baseURL).toEqual('https://example.com/from_env');
    });

    test('empty env variable', () => {
      process.env['BANDLAB_SDK_BASE_URL'] = ''; // empty
      const client = new BandlabSDK({ bearerToken: 'My Bearer Token' });
      expect(client.baseURL).toEqual('https://test.bandlab.com/api/v1.3');
    });

    test('blank env variable', () => {
      process.env['BANDLAB_SDK_BASE_URL'] = '  '; // blank
      const client = new BandlabSDK({ bearerToken: 'My Bearer Token' });
      expect(client.baseURL).toEqual('https://test.bandlab.com/api/v1.3');
    });
    ...
    test('env variable with environment', () => {
      ...
      expect(
        () => new BandlabSDK({ bearerToken: 'My Bearer Token', environment: 'production' }),
    ...
    test('in request options overridden by env variable', () => {
      process.env['BANDLAB_SDK_BASE_URL'] = 'http://localhost:5000/env';
      const client = new BandlabSDK({ bearerToken: 'My Bearer Token' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
  ...
  test('maxRetries option is correctly set', () => {
    const client = new BandlabSDK({ maxRetries: 4, bearerToken: 'My Bearer Token' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new BandlabSDK({ bearerToken: 'My Bearer Token' });
  ...
  test('with environment variable arguments', () => {
    ...
    process.env['BANDLAB_SDK_BEARER_TOKEN'] = 'My Bearer Token';
    const client = new BandlabSDK();
  ...
  test('with overridden environment variable arguments', () => {
    // set options via env var
    process.env['BANDLAB_SDK_BEARER_TOKEN'] = 'another My Bearer Token';
    const client = new BandlabSDK({ bearerToken: 'My Bearer Token' });
    expect(client.bearerToken).toBe('My Bearer Token');
  });
});
...
describe('retries', () => {
  test('retry on timeout', async () => {
    ...
    const client = new BandlabSDK({ bearerToken: 'My Bearer Token', timeout: 10, fetch: testFetch });
  ...
  test('retry count header', async () => {
    ...
    const client = new BandlabSDK({ bearerToken: 'My Bearer Token', fetch: testFetch, maxRetries: 4 });
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new BandlabSDK({ bearerToken: 'My Bearer Token', fetch: testFetch });
